...
def test_router(agent):
    ...
    model_params = {
        ...
        "input": [
            {
                "role": "user",
                "content": f"You have the following nodes at your disposal: get_division, search_internet, get_sum, end. You have to decide the sequence of nodes to travel to based on based on this objective: get sum, then divide and search on the internet. These are the results thus far: {global_state.history}",
            }
        ],
        "model": "gpt-4o-mini",
    }
...
if __name__ == "__main__":
    ...
    gpt_model = create_model(
        model_type="openai", instance_params={"api_key": os.getenv("OPENAI_API_KEY")}
    )

    tool_agent = LLMAgent(
        name="tool_agent", llm_model=gpt_model, tools=[get_result_sum]
    )
    ...
    get_division_node = Node(
        node_name="get_division", kind="STATE", command=get_division
    )

    search_internet_node = Node(
        node_name="search_internet", kind="STATE", command=search_internet
    )
    ...
    with Graph(state=graphState) as graph:
        ...
        graph.run(
            start_node=supervisor_node,
            streaming=True,
            additional_log_entries={"test": "test"},
        )
